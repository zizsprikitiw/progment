...
class Index extends CI_Controller {
	...
	public function data_info_bar()
	{
		...
		$is_distinct = 'false';
		$select = '*';
		$where = " jenis_file=2 AND (status_approval1='' AND userid_approval1='".$user_id."') OR (status_approval2='' AND userid_approval2='".$user_id."') ";
		$where_in_field = '';
		$where_in_array = array();
	...
	public function data_task_timeline()
	{
		...
		foreach ($list_task_timeline as $list_item) {			
			$data_task_timeline[] = array("id" => $list_item->id, "group" => $list_item->modul_id, "content" => $list_item->nama_task, "start" => $list_item->start_date, "end" => $list_item->due_date, "title" => 'DE: <b>'.$list_item->nama_pic.'</b> <br>Progress: <b>'.$list_item->progress.'%</b>', "className" => ($list_item->pic_id==$user_id?'bg-green-turquoise font-white':''));
		}
